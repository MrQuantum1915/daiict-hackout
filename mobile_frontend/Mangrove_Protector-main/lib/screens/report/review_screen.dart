...
import 'package:mangrove_protector/services/encryption_service.dart'; // Added import for EncryptionService
...
class _ReviewScreenState extends State<ReviewScreen> {
  ...
  Future<void> _submitReport() async {
    ...
    try {
      ...
      print('Ensuring encryption keys are available...');
      final keysReady = await authProvider.ensureEncryptionKeys();
      if (!keysReady) {
        throw Exception('Failed to ensure encryption keys are available. Please try again or log out and log back in.');
      }

      // Test encryption before submitting
      final encryptionTest = await EncryptionService.testEncryption();
      if (!encryptionTest) {
        throw Exception('Encryption service is not working properly. Please restart the app.');
      }
      ...
      if (mounted) {
        ...
        if (e.toString().contains('encryption')) {
          errorMessage = 'Encryption error: ${e.toString()}';
