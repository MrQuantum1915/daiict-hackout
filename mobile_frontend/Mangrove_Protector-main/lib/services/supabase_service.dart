...
class SupabaseService {
  ...
  static Future<IllegalActivity> createReport(IllegalActivity report) async {
    try {
      ...
      if (publicKey == null) {
        throw Exception('No public key found. Please ensure encryption keys are generated.');
      }

      // Test encryption before proceeding
      final encryptionTest = await EncryptionService.testEncryption();
      if (!encryptionTest) {
        throw Exception('Encryption test failed. Please check encryption service.');
      }

      // Encrypt the description
      String encryptedDescription;
      try {
        encryptedDescription = await EncryptionService.encryptData(
          report.description, 
          publicKey
        );
      } catch (e) {
        throw Exception('Failed to encrypt description: $e');
      }
      ...
      if (report.imageUrl != null && report.imageUrl!.isNotEmpty) {
        try {
          ...
          if (await file.exists()) {
            ...
            print('Warning: Image file not found at path: ${report.imageUrl}');
          }
        } catch (e) {
          print('Warning: Failed to add image file: $e');
